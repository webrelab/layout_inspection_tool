...
class DefaultWebScanner<E> : IScanner<E> {
    override fun scan(testingPiece: List<TestingPiece<E>>, container: Position?): List<LayoutElement<E>> {
        return testingPiece
    ...
    private fun scan(piece: TestingPiece<E>, container: Position?): List<LayoutElement<E>> {
        ...
        piece.types.forEach {
            if (it.isPositionOnly) {
                ...
                val elements = mutableListOf(piece.element)
    ...
    private fun create(element: E, type: IMeasuringType, container: Position, name: String): LayoutElement<E>? {
